...
class DemoTitleTextFieldViewController: TableViewController {
    private let textField = Components.input.defaultTextField()
        .with(placeholder: "Text")
        .with(title: "Input")
    private let disableTextField = Components.input.defaultTextField()
        .with(placeholder: "Text")
        .with(isEnabled: false)
        .with(title: "Disable")
    private let amountTextField = Components.input.defaultAmountTextField()
        .with(title: "Amount")
    private let errorTextField = Components.input.defaultTextField()
        .with(error: LoremIpsum.paragraph(sentences: 3))
        .with(placeholder: "Some text")
        .with(title: "Error")
    private let longErrorTextField = Components.input.defaultTextField()
        .with(error: LoremIpsum.paragraph(sentences: 3))
        .with(placeholder: "Some text")
        .with(title: "Long error")
    private let infoTextField = Components.input.defaultTextField()
        .with(info: LoremIpsum.paragraph(sentences: 2))
        .with(placeholder: "Some text")
        .with(title: "Info")
        .with(titleColor: UIColor.blue)
    private let infoAndErrorTextField = Components.input.defaultTextField()
        .with(error: LoremIpsum.paragraph(sentences: 2))
        .with(info: LoremIpsum.paragraph(sentences: 2))
        .with(placeholder: "Some text")
        .with(title: "Info and error")
    private let footerTextField = Components.input.defaultTextField()
        .with(text: "Some text")
        .with(title: "Footer")
