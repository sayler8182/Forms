...
public extension Mock {
    func date(from fromDate: String,
        ...
        guard !self.isNull(options) else { return nil }
        guard let fromDate: Date = fromDate.asDate else { return nil }
        guard let toDate: Date = toDate.asDate else { return nil }
        return self.date(
            from: fromDate,
            to: toDate,
            options: options)
...
public extension Mock {
    func phone(prefix: String? = nil,
        ...
        var string: String = ""
        for i in 0..<length {
            string.insert(contentsOf: i.asString, at: string.startIndex)
            if let groupingSeparator: String = groupingSeparator,
            i % 3 == 2 && (i < length - 1 || prefix.isNotNil) {
